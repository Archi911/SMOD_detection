...
st.title("🫁 UNet++ with ResNet-34 Pneumonia Detection")
...
st.sidebar.info(
    "This diagnostic tool uses **UNet++** with a **ResNet-34** backbone. "
...
st.sidebar.markdown("### Heatmap Color Legend")
st.sidebar.markdown(
    "* **🔵 Blue:** Normal / Healthy Lung Area\n"
    "* **🟡 Yellow:** Mild / Moderate Infection\n"
    "* **🔴 Red:** Severe Infection Opacity"
)
...
if uploaded_file is not None:
    with st.spinner("Analyzing X-Ray... Please wait."):
        try:
            ...
            with img_col2:
                st.image(overlay_img, caption=f"UNet++ ResNet-34 Overlay ({severity_info['category']})", use_container_width=True, channels="RGB")
